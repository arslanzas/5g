...
export const metadata: Metadata = {
  title: '5G News Pakistan | Latest Telecom Updates & Spectrum Auctions 2026',
  description: 'Stay updated with commercial 5G launch dates, PTA spectrum auctions, Jazz, Zong, and Telenor 5G trial reports, and import tax news in Pakistan.',
  robots: {
    index: true,
    follow: true,
...
export default async function NewsDirectoryPage() {
  ...
  const newsList: NewsItem[] = data?.allNews || []
    ...
    <div style={{ backgroundColor: '#F8FAFC', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#0F172A', overflowX: 'hidden' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '16px 14px 40px' }}>
        ...
        <div style={{ fontSize: '0.85rem', color: '#64748B', marginBottom: '16px' }}>
          ...
          <span style={{ fontWeight: 600 }}>5G Telecom News</span>
        </div>

        {/* Directory Banner */}
        <div style={{ backgroundColor: '#FFF', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '20px 16px', marginBottom: '24px' }}>
          <h1 style={{ fontSize: '1.6rem', fontWeight: 900, margin: '0 0 6px', color: '#0F172A' }}>
            Pakistan 5G Telecom News
          </h1>
          <p style={{ margin: 0, fontSize: '0.85rem', color: '#64748B' }}>
            Latest updates on commercial spectrum auctions, PTA policies, carrier rollouts, and mobile tech in Pakistan.
          </p>
        </div>
        ...
        <section style={{ marginBottom: '40px' }}>
          {newsList.length === 0 ? (
            <div style={{ padding: '36px 16px', textAlign: 'center', backgroundColor: '#FFF', borderRadius: '8px', border: '1px solid #E2E8F0' }}>
              <p style={{ color: '#64748B', margin: 0 }}>No news articles published yet. Check back soon!</p>
            </div>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: '14px' }}>
              {newsList.map((item) => {
                ...
                const dateString = item.publishedAt
                  ? new Date(item.publishedAt).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' })
                  : new Date(item._createdAt).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' })

                return (
                  <Link key={item._id} href={newsUrl} style={{ textDecoration: 'none', color: 'inherit' }}>
                    <div style={{ backgroundColor: '#FFF', borderRadius: '10px', border: '1px solid #E2E8F0', padding: '14px', display: 'flex', gap: '14px', alignItems: 'center', boxShadow: '0 1px 3px rgba(0,0,0,0.02)' }}>
                      <div style={{ width: '84px', height: '84px', backgroundColor: '#F1F5F9', borderRadius: '8px', overflow: 'hidden', flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        {item.imageUrl ? (
                          <img src={item.imageUrl} alt={item.title} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
                        ) : (
                          <span style={{ fontWeight: 800, color: '#10B981', fontSize: '0.85rem' }}>5G NEWS</span>
                        )}
                      </div>
                      <div style={{ flex: 1, minWidth: 0 }}>
                        ...
                        <h2 style={{ margin: '0 0 6px', fontSize: '0.95rem', fontWeight: 700, color: '#0F172A', lineHeight: 1.35, display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical', overflow: 'hidden' }}>
                          {item.title}
                        </h2>
                        {item.snippet && (
