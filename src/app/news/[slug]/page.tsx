        ...
        if (block._type === 'image' && block.asset?.url) {
          return (
            <div key={block._key} style={{ margin: '24px 0', textAlign: 'center' }}>
              <img src={block.asset.url} alt="News visual" style={{ maxWidth: '100%', borderRadius: '8px' }} />
            </div>
          )
        }
    ...
    <div style={{ backgroundColor: '#F8FAFC', minHeight: '100vh', fontFamily: 'system-ui, -apple-system, sans-serif', color: '#0F172A', overflowX: 'hidden' }}>
      ...
      <main style={{ maxWidth: '900px', margin: '0 auto', padding: '16px 14px 40px' }}>
        ...
        <div style={{ fontSize: '0.85rem', color: '#64748B', marginBottom: '16px' }}>
          ...
          <Link href="/news" style={{ color: '#0284C7', textDecoration: 'none' }}>5G News</Link> &gt;{' '}
          <span>{news.title}</span>
        </div>

        {/* News Article Content */}
        <article style={{ backgroundColor: '#FFF', borderRadius: '12px', border: '1px solid #E2E8F0', padding: '20px 16px', marginBottom: '32px' }}>
          <span style={{ display: 'inline-block', backgroundColor: '#E0F2FE', color: '#0369A1', fontSize: '0.75rem', fontWeight: 700, padding: '4px 10px', borderRadius: '4px', marginBottom: '10px' }}>
            5G Telecom Update
          </span>
          ...
          <div style={{ fontSize: '0.82rem', color: '#64748B', marginBottom: '20px' }}>
            📅 Published: {news.publishedAt ? new Date(news.publishedAt).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' }) : new Date(news._createdAt).toLocaleDateString('en-PK', { month: 'short', day: 'numeric', year: 'numeric' })}
          </div>

          {news.imageUrl && (
            <div style={{ width: '100%', borderRadius: '8px', overflow: 'hidden', marginBottom: '22px', maxHeight: '420px', backgroundColor: '#F1F5F9' }}>
              <img src={news.imageUrl} alt={news.title} style={{ width: '100%', height: 'auto', display: 'block', objectFit: 'cover' }} />
            </div>
          )}
